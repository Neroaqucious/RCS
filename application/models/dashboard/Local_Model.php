...
class Local_Model extends CI_Model
{
  ...
  public function getLocal()
	{
    $this->db->select('local.id,local.name,requirement,subject,photo,thumb,local.created_by,local.updated_by,local.created_at,local.updated_at,local.status,tag.name as tag');
  ...
  public function getlocalsDetail($id)
  {
    $this->db->select('local.id,local.name,subject,requirement,thumb,photo,address,phone,email,tag_id,local.updated_at,local.created_at,local.status,tag.name as tag');
    $this->db->where($this->db2.'.id', $id);
    $this->db->join($this->db1, $this->db1.'.id = '.$this->db2.'.tag_id', 'left' );
    return $this->db->get($this->db2)->row();
  }

	public function localCheck($data, $id = null)
	{
	...
	public function localInsert($data)
	{
    $this->db->insert($this->db2, $data);
    return true;
  }
  
  public function localsUpdate($data, $id)
  {
    $this->db->where('id', $id);
	...
	public function localsDelete($id)
	{
		$this->db->where('id', $id);
