...
defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Jpschool extends CI_Controller
	{
		private $private_db = "dashboard/Jpschool_Model"; 
		...
		{
			$this->Header = array(
				'title' => "Japan School Management",
				'uri' => array("information","jpschool_list"),
				'respond' => array(
			...
			$result = $this->Jpschool_Model->getSchool();
			...
			$this->load->view('dashboard/jp/list', $this->data);
		}
		...
		public function add()
		{
			$this->Header = array(
				...
				'uri' => array("jpschool","jpschool_list"),
				'respond' => array(
					'list' => 'admin/school',
					'tags' => $this->Jpschool_Model->getTagsData(),
				),
			);
			$respondheader = $this->configlibrary->__SetDataMargeRecursive($this->globalHeader, $this->Header);
			...
			if($_POST) {
				...
				if ($this->form_validation->run() === false) {
					$this->load->view('dashboard/jp/add', $this->data);
				} else {
					$data = array(
            ...
            'updated_by' => $this->current_id,
					...
					$checkdata = $this->Jpschool_Model->schoolCheck($data);
					if ($checkdata){
					...
					$this->Jpschool_Model->schoolInsert($data);
		...
		public function edit($id)
		{
			...
			$respondheader = $this->configlibrary->__SetDataMargeRecursive($this->globalHeader, $this->Header);
			$this->data['result'] = $this->Jpschool_Model->getjpschoolsDetail($id);
			/** Validation User data **/
			$this->securitylibrary->__MatchEmptyValid($id, $this->data['result'], "/admin/school", "Not allow!");			
			/** User data marge **/
			$this->data = $this->configlibrary->__SetDataMarge($respondheader, $this->data);
			...
			if($_POST) {
				...
				if ($this->form_validation->run() === false) {
					$this->load->view('dashboard/jp/edit', $this->data);
				} else {
					$data = array(
						'name' => $this->input->post('name'),
						'email' => $this->input->post('email'),
            ...
            'updated_by' => $this->current_id,
					...
					$data = $this->securitylibrary->__Match_Xss($data);
					$checkdata = $this->Jpschool_Model->schoolCheck($data, $id);
					...
					$this->Jpschool_Model->jpschoolsUpdate($data, $id);
		...
		public function view($id)
		{
			...
			/** User data marge **/
			$this->data = $this->configlibrary->__SetDataMarge($respondheader, $this->data);
			$this->load->view('dashboard/jp/view', $this->data);
		}
		...
		public function delete($id)
		{
			 ...
			 * Jpschool GET/
			 **/		
			$list = $this->Jpschool_Model->getjpschoolsDetail($id);
			/** Validation User data **/
			
			/** Validation User data **/
			$this->securitylibrary->__MatchEmptyValid($id, $list, "/admin/school", "Not allow!");	
			...
			$this->Jpschool_Model->jpschoolsDelete($id);
